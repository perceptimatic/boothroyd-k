...
# Calculates the k value at a single point using the estimate (e_c) = (e_z)^k for the given datasets 

export PYTHONUTF8=1
[ -f "path.sh" ] && . "path.sh"

usage="Usage: $0 [-h] [-D] [-S STR] [-O STR] [-d DIR] [-p DIR] [-P FILE]
[-o DIR] [-n DIR] [-N DIR] [-t DIR] [-s INT] [-l NAT] [-x FILE] [-k STR]"
delete_wavs=false
decode_script=
decode_script_opts=
data=
...
Options
    ...
    -s INT      The signal-to-noise ratio (SNR) in dB (default: '$snr')
...
while getopts "hDS:O:d:p:P:o:n:N:t:s:l:x:k:" name; do
    case $name in
        h)
            echo "$usage"
            echo ""
...
for part in "${partitions[@]}"; do
    ...
    echo "HP"
    python3 get_single_k.py "$spart/1/ref.trn" "$spart/1/hyp.trn" "$spart/3/ref.trn" "$spart/3/hyp.trn"
    echo "LP"
    python3 get_single_k.py "$spart/2/ref.trn" "$spart/2/hyp.trn" "$spart/3/ref.trn" "$spart/3/hyp.trn"
done